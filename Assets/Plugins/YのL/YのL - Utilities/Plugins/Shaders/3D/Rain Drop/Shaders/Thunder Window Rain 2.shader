Shader "Unlit/Test Rain"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float4 iResolution = (_ScreenParams.xy, 0, 0);
                float4 iMouse = float4(_Time.y * 60.0, _ScreenParams.xy, 0.0);

                float3 M = 0;//iMouse.xyz / iResolution.xyz;
	            float2 uv = i.uv;
                float2 UV = i.uv;
                ...
                float3 col = tex2Dlod(_MainTex, float4(UV + n, focus, 0)).rgb;
    
    
                #ifdef USE_POST_PROCESSING
                t = (T+3.)*.5;										// make time sync with first lightnoing
                float colFade = sin(t*.2)*.5+.5+story;
                col *= lerp((1.), float3(.8, .9, 1.3), colFade);	// subtle color shift
                ...
                #ifdef HAS_HEART
    	            col = lerp(pow(col, (1.2)), col, heart);
    	            fade *= S(102., 97., T);
                #endif
